...
describe('GET /company/:companyId/workshifts/:workshiftId/attendances', () => {
    ...
    describe('Without workshiftMemberId parameter', () => {
        for (const role of roles) {
            const success = role !== 'MEMBER'
            const shouldOrNot = success ? 'should' : 'should not'
            const expected = success ? 200 : 403
            it(`${shouldOrNot} get all attendance in workshift for role ${expected}`, async () => {
                await createMember(setupVar.user1.id, company.id, role)
                const res = await supertest(app).get(`/company/${company.id}/workshifts/${workshift.id}/attendances`)
        ...
        it('should get all attendance in workshift for admin', async () => {
            const res = await supertest(app).get(`/company/${company.id}/workshifts/${workshift.id}/attendances`)
    ...
    describe('With workshiftMemberId parameter', () => {
        beforeAll(async () => {
        ...
        for (const role of roles) {
            ...
            const shouldOrNot = success ? 'should' : 'should not'
            const expected = success ? 200 : 403
            it(`${shouldOrNot} get all attendance in for specific workshiftMember for role ${expected}`, async () => {
                await createMember(setupVar.user1.id, company.id, role)
                const res = await supertest(app).get(`/company/${company.id}/workshifts/${workshift.id}/attendances`)
                    .set('Authorization', `Bearer ${setupVar.user1.session.token}`)
                    .query({ workshiftMemberId: attendance.workshiftMemberId })
                expect(res.statusCode).toBe(expected)
            })
        }

        it('should get all attendance in for specific workshiftMember for admin', async () => {
            const res = await supertest(app).get(`/company/${company.id}/workshifts/${workshift.id}/attendances`)
                .set('Authorization', `Bearer ${setupVar.admin.session.token}`)
                .query({ workshiftMemberId: attendance.workshiftMemberId })
            expect(res.statusCode).toBe(200)
        ...
        it('should get all attendance in for specific workshiftMember for creator', async () => {
            const res = await supertest(app).get(`/company/${company.id}/workshifts/${workshift.id}/attendances`)
